...
An L<opcode|#opcode> is a numeric identifier that specifies an instruction to be performed.
...
A method stub is a piece of code that is used to simulate the behavior of existing code or as a placeholder for code that is not yet implemented.
...
An L<unspace|#unspace> allows language extensibility by telling the compiler's lexer to ignore contiguous whitespace. An unspace is a backslash followed by whitespace characters.

    # An unspace can be used to line up postfix operators. 
